...
void bfs(int size,int adj[size][size],int start){
    ...
    if(!init_circ_queue(size+1,&nodes)){            //initializing circular queue with size+1 to maintain circular queue invariant (one slot empty)
    ...
    visited[start]=1;
    enqueue(&nodes,start);
    while(1){                                   //main loop which performs bfs
