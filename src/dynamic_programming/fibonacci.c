...
void fibonacci_demo(void)
{
    while (1)
    {
        ...
        long long res_memo = fibonacci_memoization(n, memo);
        clock_t end_m = clock();
        double time_memo = (double)(end_m - start_m) / CLOCKS_PER_SEC;

        clock_t start_t = clock();
        long long res_tab = fibonacci_tabulation(n);
        clock_t end_t = clock();
        double time_tab = (double)(end_t - start_t) / CLOCKS_PER_SEC;

        printf("\n--- Results ---\n");
        printf("Fibonacci(%d) via Memoization: %lld (Time: %f seconds)\n", n, res_memo, time_memo);
        printf("Fibonacci(%d) via Tabulation : %lld (Time: %f seconds)\n", n, res_tab, time_tab);

        free(memo);

        add_to_history("Fibonacci Memoization", n, time_memo);
        add_to_history("Fibonacci Tabulation", n, time_tab);
    }
}
