...
void bfs_demo(void)
{
    while (1)
    {
        ...
        { // nested for loop to take adjacency matrix at runtime
            for (int j = 0; j < graph_capacity; j++)
            {
            bfs_node_start:
                printf("\nenter the value of %d row of %d column, (either 1 or 0), enter '-1' to "
                       "exit : ",
                       i, j);
...
void bfs(int size, int adj[size][size], int start)
{
    ...
    { // initializing circular queue with size+1 to maintain circular queue invariant (one slot
      // empty)
        printf("queue could not be initialized due to a malloc failure");
